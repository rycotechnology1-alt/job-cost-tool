# Transition Tracker Archive

This archive preserves the retired long-form migration tracker and older change history.
Use [docs/transition_tracker.md](docs/transition_tracker.md) as the live tracker first.
Open this archive only when an older architecture, workflow, or migration decision matters.

---

### [2026-04-09] Web settings now hides draft lifecycle terms behind a simpler current-profile edit/save flow
- **What changed:** Simplified the browser settings workspace so operators now see `Edit current profile` in read-only mode and `Save profile settings` while editing. The top save action now batches dirty Phase 2A section saves through the existing patch endpoints and immediately publishes through the existing publish endpoint, while the separate top-level publish button and visible per-section save buttons were removed. Updated leave-settings prompts and status copy to talk about live profiles and unpublished profile changes instead of drafts, and expanded browser regressions to prove the old draft/publish wording is gone from the main workflow while save-and-leave now saves plus publishes before navigation.
- **Why:** The previous create/open/publish draft terminology exposed too much backend lifecycle detail to operators, added avoidable clicks, and made profile settings feel harder to trust even when the underlying system behavior was correct.
- **Area:** Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that advanced operators lose some explicit visibility into the hidden draft lifecycle, though the backend model, validation, lineage, discard behavior, and published-version processing rules remain unchanged and the browser flow now better matches the operator mental model.
- **Follow-up needed:** If profile-management ergonomics are revisited later, keep future simplifications in this same operator-facing language and avoid reintroducing backend draft jargon unless a genuinely admin-only surface needs it.

### [2026-04-09] Web settings now blocks unsafe leave-with-unsaved-draft flows and no longer restores false empty draft state
- **What changed:** Fixed the browser settings remount bug that could capture an empty local editor as fake retained draft edits and later reopen a saved draft looking blank. Added a narrow draft-discard API/service seam, hardened draft hydration before any retained browser snapshot is written, and introduced an explicit settings leave dialog for review-workspace switches and trusted-profile switches with `Save and leave`, `Don't save`, and `Stay here` actions. Expanded browser regressions for the reported false-empty-draft flow plus save/discard leave behavior, and added backend tests covering discard without published-version mutation or observation loss.
- **Why:** Operators could leave profile settings, come back, reopen the current draft, and see saved mappings/rates/classifications appear gone even though the draft was still recoverable, which was a major trust-eroding bug. The settings shell also needed a clearer decision point before abandoning unsaved draft work.
- **Area:** Application services / Persistence/API prep / Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that discarding a draft is now more explicit and irreversible for unpublished changes, though the prompt makes that choice deliberate, published versions remain untouched, and unresolved observations can still recreate a fresh draft later when processing finds them again.
- **Follow-up needed:** If later settings hardening is requested, decide separately whether browser/tab-close warnings should use the same save-vs-discard model; do not widen this into broader profile-admin lifecycle work without an explicit scope change.

### [2026-04-08] Uploaded source PDFs now expire from temporary web storage after a short retention window
- **What changed:** Added a backend-managed upload retention setting with a default 24-hour TTL, extended the local runtime storage seam to timestamp new uploads and opportunistically delete expired upload directories on startup/save/load, and surfaced a clear expired-upload message when a stale cached `upload_id` is used to create a processing run. Added focused storage/API/browser regressions covering expired cleanup, legacy upload fallback via filesystem timestamps, cleanup-disabled behavior, stale upload failure messaging, and the rule that existing run/review lineage still works after upload cleanup.
- **Why:** The web runtime was persisting every uploaded PDF indefinitely under `runtime/api/uploads` even though uploads are only a temporary cache for processing and short-lived rerun convenience.
- **Area:** Persistence/API prep / Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that operators must reupload a PDF if they wait beyond the retention window before creating a new run from a cached browser upload token, which is intentional; persisted processing runs, review sessions, exports, and source-document lineage metadata remain untouched.
- **Follow-up needed:** If later workflow needs real source-PDF retrieval after processing, revisit upload retention explicitly instead of silently relying on this temporary-cache path.

### [2026-04-08] Review classification edits now stay constrained to run-bound target classifications in web
- **What changed:** Extended the review-session API to return labor/equipment classification options from the immutable processing run's profile snapshot, replaced the web review editor's freeform labor/equipment text inputs with controlled dropdowns plus a blank option like desktop, and hardened the shared review update seam so server-side review edit submission now rejects any labor or equipment classification label that is not in that run snapshot's trusted-profile target lists. Added focused service/API/browser regressions for valid dropdown submission, invalid classification rejection, and profile-specific option sets.
- **Why:** Web review edits were still allowing arbitrary typed labor and equipment classifications, which diverged from desktop behavior and let the server trust invalid values that did not exist in the trusted profile configuration used to process the run.
- **Area:** Application services / Persistence/API prep / Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that previously saved browser-only invalid review overlays from older dev runs may now surface as legacy invalid selections until the user chooses a configured classification or reruns, though new submissions fail closed and the browser no longer offers freeform entry.
- **Follow-up needed:** If later review-editor parity passes are requested, the next safe step would be hiding irrelevant labor vs equipment selectors based on row type without weakening the run-snapshot validation gate.

### [2026-04-08] Review export now invalidates immediately when the selected trusted profile changes after processing
- **What changed:** Hardened the browser review workspace so an active processed review becomes stale for export as soon as the selected trusted profile changes. The web shell now latches that profile-switch invalidation on the active review context, clears the in-browser export summary, disables export immediately, and shows a clear message that the loaded review was processed under a different profile selection and must be reprocessed before export is allowed again. Added a focused browser regression proving profile switch disables export until a fresh processing run is created under the newly selected profile.
- **Why:** Operators could previously process and edit a report under one trusted profile, switch the selected profile in the browser, and still export the old review without rerunning, which undermined the run/profile lineage boundary and made the review context look fresher than it really was.
- **Area:** Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk; this is a conservative browser-side guard that uses the processing run's trusted-profile context as the export gate and intentionally requires rerun once profile selection changes during an active review, even if the user later switches back.
- **Follow-up needed:** If later review hardening is requested, the next step could be surfacing more run-profile snapshot detail in the browser for extra trust, but this bug is fixed without widening export/template scope or changing backend lineage rules.

### [2026-04-07] Multi-profile settings now keeps browser-only draft edits isolated per profile and makes current draft-vs-published state clearer
- **What changed:** Hardened the browser settings workspace so switching between trusted profiles now clears the active editor immediately, keeps browser-only unsaved edits attached to the originating profile/draft within the current tab, and restores those edits only when that same draft is reopened. Added stronger trust signals in the settings header and status strip for selected profile identity, source kind, published version, current workspace mode (published vs draft), open-draft state, and whether local unsaved edits are retained for another profile in the current tab. Expanded browser regressions to cover cross-profile switching with different unsaved local edits, draft reopen isolation, status updates after draft open/publish, and restore followed by clean reselection.
- **Why:** After create/archive/restore landed, the remaining trust gap was active editing safety. The workspace still behaved like one shared local editor, which made it too easy to worry that draft changes or unsaved browser state might leak across profiles or be silently discarded during normal multi-profile work.
- **Area:** Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that the settings workspace now keeps a little more browser-local draft state per profile in the active tab, though the backend remains authoritative, published versions still drive processing, and reopening a draft only reapplies retained local edits when the saved draft identity still matches.
- **Follow-up needed:** Still inside current Phase 2A scope, a later pass could add even more explicit per-profile session summaries or a dedicated “reopen retained draft edits” affordance if operators want less reliance on the existing open-draft button. Rename and broader lifecycle/admin work remain deferred.

### [2026-04-07] One-org profile management now supports archived-profile restore and field-level create conflict surfacing
- **What changed:** Added archived-profile visibility through the trusted-profile read-only summary contract, a narrow restore/unarchive action for user-created archived profiles, and browser settings UI that shows archived profiles separately with restore affordances while keeping them out of active review/profile selectors until restored. Also upgraded create-profile conflict mapping so backend identity conflicts return structured field errors and the browser can surface duplicate-key or duplicate-display-name issues directly on the matching form fields.
- **Why:** After archive landed, the next highest-value low-risk gap was making archived profiles practical to manage without drifting into delete/admin tooling, while improving trust in create-profile failures that still only showed up as a top-level error banner.
- **Area:** Application services / Persistence/API prep / Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that the settings workspace now does one extra archived-profile summary fetch; archived profiles still remain non-processable for new web work until restored, and restore preserves the original logical identity plus published-version lineage instead of recreating the profile.
- **Follow-up needed:** Rename remains deferred because stable logical ids and lineage identity still matter more than browser convenience in this slice. If rename is revisited later, it should be limited to display metadata only unless there is a very explicit identity-safe design.

### [2026-04-07] One-org multi-profile settings now validates creation, exposes selector state, and archives user-created profiles safely
- **What changed:** Tightened trusted-profile creation validation with case-insensitive duplicate checks for stable profile keys and active display names, exposed source/open-draft/current-version state in trusted-profile listings, added a narrow archive action for user-created published-clone profiles with no open draft, and hardened the browser settings selector so opening a draft no longer clears itself through summary-refresh churn and switching profiles warns before unsaved browser edits are discarded. Expanded API/service/repository/browser regressions for duplicate creation rejection, clean clone-backed `v1` initialization, archive behavior, selector visibility, and safe profile switching.
- **Why:** After second-profile creation landed, the remaining trust gap was lifecycle safety: operators still needed clearer multi-profile state, duplicate/conflict protection, and a safe way to remove disposable web-created profiles without violating published-version lineage or processing auditability.
- **Area:** Application services / Persistence/API prep / Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that archived profiles are hidden from active selectors while remaining in lineage history, which is intentional; hard delete remains avoided because it would create unnecessary pressure on published-version and processing-run audit chains.
- **Follow-up needed:** Still inside current Phase 2A scope, a later pass could add richer server-returned field-level creation errors or archived-profile inspection if operators need it; rename/delete beyond this archive model remains deferred until there is a stronger lifecycle need.

### [2026-04-07] Phase 2A settings now supports creating and publishing a second trusted profile inside the same organization
- **What changed:** Added a narrow profile-authoring create path that seeds a new logical trusted profile from an existing current published version, persists that clone immediately as the new profile's immutable published version `v1`, and then reuses the existing single-draft/save/publish flow for Phase 2A settings edits. Switched trusted-profile listing to the persisted repository-backed source so user-created profiles appear in both web selectors, added a lean browser create-profile form in the settings workspace, and added repository/service/API/browser regressions for create-list-open-save-publish behavior on a second profile.
- **Why:** The Stage 6/7 settings workspace could only operate on already-bootstrapped filesystem profiles. That meant a second profile could not be created and saved in web because there was no creation endpoint, no persisted listing path for non-filesystem profiles, and no way for a brand-new logical profile to get its first published version before draft creation.
- **Area:** Application services / Persistence/API prep / Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that operators may assume new profiles clone in-progress draft edits, though the UI now states clearly that creation seeds from the selected published version only. Filesystem bootstrap repair remains limited to filesystem-backed profiles so user-created profiles do not fall back to implicit disk bootstrap behavior.
- **Follow-up needed:** Still inside current Phase 2A scope, a later pass could add richer profile-creation validation messaging or explicit rename/delete affordances for persisted profiles; making deferred domains writable or introducing broader org/user management remains out of scope.

### [2026-04-07] Phase 2A settings workspace now preserves unsaved browser edits across section saves and hardens draft status handling
- **What changed:** Tightened the existing browser settings workspace so Phase 2A section saves no longer wipe unsaved browser edits in other editable domains, publish is blocked until unsaved sections are saved, inline field validation and empty states are surfaced inside the approved writable domains, and the published-vs-draft status is clearer. Also added a narrow browser-side authoring client error type plus stale-open-draft recovery that falls back to the existing create/open endpoint when the published detail points at a missing draft id, and publish now re-reads published profile detail after success for better refresh confidence.
- **Why:** The first Stage 6/7 workspace was functional, but it still had trust-eroding UX seams inside the approved Phase 2A slice: section saves could silently clobber unrelated edits, publish readiness was too opaque, draft/open failures were brittle, and operators had too little confidence about what was saved versus still local to the browser.
- **Area:** Web delivery / Tests
- **Portability impact:** Increased
- **Risks introduced:** Low risk that the browser now owns a bit more local editor-state bookkeeping, though it still stays thin, reuses the same narrow authoring endpoints, keeps deferred domains read-only, and leaves published-version processing rules unchanged.
- **Follow-up needed:** If later Phase 2A polish is requested, stay focused on in-scope editor ergonomics such as more per-section retry affordances or richer draft summaries; do not widen deferred-domain edit scope.
